...
tfidf_file_path = os.path.join(output_folder, '1grams_tfidf.json')
...
# 前80回目用红色点表示
plt.scatter(embedding[:80, 0], embedding[:80, 1], color='red', label='前80回')

# 后40回目用蓝色点表示
plt.scatter(embedding[80:, 0], embedding[80:, 1], color='blue', label='后40回')

plt.title('《红楼梦》章节的UMAP降维结果', fontproperties=font_prop)
...
# 保存散点图
plt.savefig(os.path.join(output_folder, 'umap_1grams_scatter.png'), dpi=300, bbox_inches='tight')
plt.show()

print("UMAP 降维和散点图生成完成")
