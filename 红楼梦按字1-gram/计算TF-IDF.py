...
ngram_count_file_path = os.path.join(output_folder, 'char_1grams_count.json')

with open(ngram_count_file_path, 'r', encoding='utf-8') as file:
    ngram_count = json.load(file)

# 计算每个 1-gram 的文档频率（DF）
df = defaultdict(int)
...
# 将结果写入文件
tfidf_output_file_path = os.path.join(output_folder, 'char_1grams_tfidf.json')
