...
for chapter in range(1, 121):
    ngram_file_path = os.path.join(output_folder, f'chapter-{chapter}-pos-1grams.txt')
    with open(ngram_file_path, 'r', encoding='utf-8') as ngram_file:
        for line in ngram_file:
            n_gram = line.strip()
            ngram_count[n_gram][f'chapter-{chapter}'] += 1

output_count_file_path = os.path.join(output_folder, 'pos_1grams_count.json')
